...
tr ' ' '\n' < "../analysis/results_dropout/"$filename > "../analysis/results_dropout/"$filename".LRTs"
# use awk to convert it out of scientific notation
awk '{printf "%8.6f\n", $1}' "../analysis/results_dropout/"$filename".LRTs" > "../analysis/results_dropout/"$filename".LRTs.nosci"
#paste it next to a file with just the ENST gene names
paste "../data/10_genelist/"$genelist "../analysis/results_dropout/"$filename".LRTs.nosci" > "../analysis/results_dropout/"$output
